...
class ACFSWorktreeManager:
    ...
    def __init__(self, repo_root: str | Path = ".", max_worktrees: int = _DEFAULT_MAX_WORKTREES, worktree_base_dir: str | Path | None = None, auto_cleanup_hours: int = _DEFAULT_AUTO_CLEANUP_HOURS) -> None:
    ...
    def create_worktree(self, agent_id: str, branch_name: str) -> str:
        ...
        try:
            self._run_git(["worktree", "add", "-b", branch_name, str(wt_dir)], check=True)
        except subprocess.CalledProcessError:
            try:
                self._run_git(["worktree", "add", str(wt_dir), branch_name], check=True)
            except subprocess.CalledProcessError as e:
                raise RuntimeError(f"ACFS: failed to create worktree: {e}") from e
        wt_path = str(wt_dir.resolve())
        info = WorktreeInfo(path=wt_path, branch=branch_name, agent_id=agent_id)
        self._worktrees[wt_path] = info
        self._log_align("ACFS_WORKTREE_CREATED", {"agent_id": agent_id, "branch": branch_name, "path": wt_path})
    ...
    def destroy_worktree(self, worktree_path: str) -> None:
        ...
        try:
            self._run_git(["worktree", "remove", "--force", worktree_path], check=True)
        except subprocess.CalledProcessError as e:
            raise RuntimeError(f"ACFS: failed to remove worktree: {e}") from e
        self._worktrees.pop(worktree_path, None)
        self._log_align("ACFS_WORKTREE_DESTROYED", {"agent_id": agent_id, "path": worktree_path})
        logger.info(f"ACFS: destroyed worktree at {worktree_path}")

    def list_worktrees(self) -> list[dict[str, Any]]:
        return [{"path": info.path, "branch": info.branch, "agent_id": info.agent_id, "created_at": info.created_at, "commit_count": info.commit_count} for info in self._worktrees.values()]

    def shadow_commit(self, worktree_path: str, message: str) -> str:
        try:
            self._run_git(["add", "-A"], cwd=worktree_path, check=True)
            diff_output = self._run_git(["diff", "--cached", "--stat"], cwd=worktree_path)
            merkle_hash = hashlib.sha256(diff_output.encode() if diff_output else b"").hexdigest()[:16]
            full_message = f"{message}\n\nALIGN-Merkle: {merkle_hash}"
            self._run_git(["commit", "-m", full_message, "--allow-empty"], cwd=worktree_path, check=True)
        ...
        if info:
            info.commit_count += 1
        self._log_align("ACFS_SHADOW_COMMIT", {"path": worktree_path, "agent_id": info.agent_id if info else "unknown", "sha": sha, "merkle_hash": merkle_hash, "message": message})
    ...
    def diff_worktree(self, worktree_path: str) -> list[dict]:
        try:
            output = self._run_git(["diff", "--name-status", "HEAD~1..HEAD"], cwd=worktree_path)
        except Exception:
            output = self._run_git(["status", "--porcelain"], cwd=worktree_path)
        changes = []
    ...
    def detect_conflicts(self, wt_a: str, wt_b: str) -> dict:
        ...
        for filepath in overlap:
            detail = {"file": filepath, "regions_a": [], "regions_b": [], "auto_resolvable": False}
            for label, wt_path in [("a", wt_a), ("b", wt_b)]:
                try:
                    diff = self._run_git(["diff", "HEAD~1..HEAD", "--unified=0", "--", filepath], cwd=wt_path)
                    regions = self._parse_diff_regions(diff)
                    detail[f"regions_{label}"] = regions
                except Exception:
                    logger.debug("Failed to collect diff for %s in worktree %s", filepath, label)
        ...
        self._log_align("ACFS_CONFLICT_DETECTION", {"wt_a": wt_a, "wt_b": wt_b, "conflicting_files": list(overlap), "safe_a": len(safe_a), "safe_b": len(safe_b)})
        return {"conflicting_files": list(overlap), "safe_files_a": list(safe_a), "safe_files_b": list(safe_b), "overlap_details": overlap_details, "all_auto_resolvable": all(d["auto_resolvable"] for d in overlap_details) if overlap_details else True}
    ...
    def merge_worktree(self, worktree_path: str, target_branch: str = "main") -> dict:
        ...
        try:
            ...
            result = subprocess.run(["git", "merge", source_branch, "--no-edit"], cwd=str(self.repo_root), capture_output=True, text=True)
            if result.returncode == 0:
                sha = self._run_git(["rev-parse", "HEAD"]).strip()
                files_output = self._run_git(["diff", "--name-only", "HEAD~1..HEAD"])
                files_changed = [f for f in files_output.strip().splitlines() if f.strip()]
                self._log_align("ACFS_MERGE_SUCCESS", {"source": source_branch, "target": target_branch, "sha": sha, "files_changed": len(files_changed)})
                return {"success": True, "merge_sha": sha, "conflicts": [], "files_changed": files_changed}
            else:
                conflicts = []
                if result.stdout:
                ...
                self._log_align("ACFS_MERGE_CONFLICT", {"source": source_branch, "target": target_branch, "conflicts": conflicts})
                return {"success": False, "merge_sha": None, "conflicts": conflicts, "files_changed": []}
        except subprocess.CalledProcessError as e:
            self._run_git(["merge", "--abort"])
            return {"success": False, "merge_sha": None, "conflicts": [str(e)], "files_changed": []}
    ...
    @staticmethod
    def _regions_overlap(regions_a: list[tuple[int, int]], regions_b: list[tuple[int, int]]) -> bool:
    ...
    def _run_git(self, args: list[str], cwd: str | None = None, check: bool = False) -> str:
        result = subprocess.run(["git", *args], cwd=cwd or str(self.repo_root), capture_output=True, text=True, check=check)
