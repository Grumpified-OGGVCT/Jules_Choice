...
5. Gas-accounted: each adjudication costs 2 units.
"""
...
def get_agent_profile() -> dict[str, Any]:
    return {
        ...
        "restrictions": {"max_tokens": 4096, "temperature": 0.0, "gas_per_adjudication": ADJUDICATE_GAS_COST},
    }
...
def adjudicate(event_type: str, payload: str | dict[str, Any]) -> ArbiterVerdict:
    ...
    if blocked:
        _logger.log("ARBITER_ALIGN_BLOCK", {"rule_id": rule_id, "event_type": event_type}, anomaly_score=0.95)
        return ArbiterVerdict(verdict=VERDICT_QUARANTINE, rule_id=rule_id, justification=f"ALIGN Ledger rule {rule_id} matched", event_type=event_type)
    if event_type == "SECURITY_ALERT":
        data = payload if isinstance(payload, dict) else {}
        severity = data.get("severity", "HIGH")
        src_rule = data.get("rule_id", "")
        if severity == "CRITICAL":
            return ArbiterVerdict(verdict=VERDICT_QUARANTINE, rule_id=src_rule, justification=f"Critical security alert: {src_rule}", event_type=event_type)
        return ArbiterVerdict(verdict=VERDICT_ESCALATE, rule_id=src_rule, justification=f"Security alert escalated: {src_rule}", event_type=event_type)
    if event_type == "BUDGET_GATE":
        data = payload if isinstance(payload, dict) else {}
        pct = float(data.get("pct", 0.0))
        if pct >= _BUDGET_QUARANTINE_PCT:
            return ArbiterVerdict(verdict=VERDICT_QUARANTINE, justification=f"Token budget at {pct:.1%} — emergency quarantine", event_type=event_type)
        if pct >= _BUDGET_ESCALATE_PCT:
            return ArbiterVerdict(verdict=VERDICT_ESCALATE, justification=f"Token budget at {pct:.1%} — escalation required", event_type=event_type)
        return ArbiterVerdict(verdict=VERDICT_ALLOW, justification=f"Token budget at {pct:.1%} — within limits", event_type=event_type)
    _logger.log("ARBITER_UNKNOWN_EVENT", {"event_type": event_type, "preview": text[:80]}, anomaly_score=0.3)
    return ArbiterVerdict(verdict=VERDICT_ESCALATE, justification=f"Unknown event type: {event_type}", event_type=event_type)
...
def _consume_loop(stop_event: threading.Event) -> None:
    try:
        ...
        from agents.gateway.router import consume_gas
        while not stop_event.is_set():
            try:
                messages = r.xreadgroup(_CONSUMER_GROUP, "arbiter-1", {ARBITER_EVENTS_STREAM: ">"}, count=1, block=2000)
                if not messages:
                    continue
                for _stream, entries in messages:
                    for entry_id, data in entries:
                        ...
                        try:
                            ...
                            verdict = adjudicate(et, event_data)
                            _logger.log("ARBITER_VERDICT", {"verdict": verdict.verdict, "rule_id": verdict.rule_id, "event_type": verdict.event_type, "justification": verdict.justification}, anomaly_score=0.9 if verdict.verdict == VERDICT_QUARANTINE else 0.0)
...
def start() -> None:
    ...
    if _thread and _thread.is_alive():
        return
    _stop_event.clear()
    _thread = threading.Thread(target=_consume_loop, args=(_stop_event,), daemon=True, name="arbiter-agent")
    _thread.start()
