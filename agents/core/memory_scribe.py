...
def prune_old_facts(conn: sqlite3.Connection) -> int:
    ...
    oldest_rowid_cutoff = conn.execute(
        "SELECT MAX(rowid) FROM fact_store WHERE rowid <= (SELECT MIN(rowid) + (MAX(rowid) - MIN(rowid)) * ? / 100.0 FROM fact_store)",
        (prune_percentile,),
    ).fetchone()[0]
    ...
    archive_path.write_text(
        _json.dumps([{"entity_id": r[1], "vector_embedding": r[2], "semantic_relationship": r[3], "confidence_score": r[4]} for r in rows])
    )
