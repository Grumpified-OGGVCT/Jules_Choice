...
def get_agent_profile() -> dict[str, Any]:
    return {
        ...
        "restrictions": {"max_tokens": 2048, "temperature": 0.0, "gas_per_audit": AUDIT_GAS_COST, "budget_gate_pct": BUDGET_GATE_PCT},
    }


def audit_budget(conn: sqlite3.Connection | None = None, db_path: Path | None = None, max_context_tokens: int | None = None) -> BudgetStatus:
    ...
    def _run(c: sqlite3.Connection) -> BudgetStatus:
        ...
        return BudgetStatus(tokens_used=used, budget=budget, pct=pct, gate_blocked=pct >= BUDGET_GATE_PCT)
...
def _publish_budget_alert(r: Any, status: BudgetStatus) -> None:
    try:
        r.xadd(ARBITER_EVENTS_STREAM, {"data": json.dumps({"event_type": "BUDGET_GATE", "source_agent": "scribe", "tokens_used": status.tokens_used, "budget": status.budget, "pct": round(status.pct, 4), "ts": time.time()})})
    except Exception as exc:
...
def _consume_loop(stop_event: threading.Event) -> None:
    try:
        ...
        while not stop_event.is_set():
            try:
                messages = r.xreadgroup(_CONSUMER_GROUP, "scribe-1", {SCRIBE_EVENTS_STREAM: ">"}, count=1, block=2000)
                if not messages:
                    continue
                for _stream, entries in messages:
                    for entry_id, _data in entries:
                        try:
                            ...
                            _logger.log("SCRIBE_AUDIT_COMPLETE", {"tokens_used": status.tokens_used, "budget": status.budget, "pct": round(status.pct, 4), "gate_blocked": status.gate_blocked})
                        except Exception as exc:
...
def start() -> None:
    ...
    if _thread and _thread.is_alive():
        return
    _stop_event.clear()
    _thread = threading.Thread(target=_consume_loop, args=(_stop_event,), daemon=True, name="scribe-agent")
    _thread.start()
