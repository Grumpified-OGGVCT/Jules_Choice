...
3. Publish ALERT events to arbiter_events Redis Stream.
4. Consume sentinel_events stream for reactive scanning.
5. Gas-accounted: each scan costs 1 unit.
"""
...
def get_agent_profile() -> dict[str, Any]:
    return {
        ...
        "restrictions": {"max_tokens": 2048, "temperature": 0.0, "gas_per_scan": SCAN_GAS_COST},
    }
...
def scan_payload(payload: str | dict[str, Any]) -> SentinelVerdict:
    ...
    if blocked:
        _logger.log("SENTINEL_BLOCKED_ALIGN", {"rule_id": rule_id, "preview": text[:120]}, anomaly_score=0.9)
        return SentinelVerdict(blocked=True, rule_id=rule_id, severity="HIGH", source="align")
    for pattern, rid in _COMPILED_PRIVESC:
        if pattern.search(text):
            _logger.log("SENTINEL_BLOCKED_PRIVESC", {"rule_id": rid, "preview": text[:120]}, anomaly_score=0.85)
...
def _publish_alert(r: Any, verdict: SentinelVerdict, original_payload: str) -> None:
    try:
        r.xadd(ARBITER_EVENTS_STREAM, {"data": json.dumps({"event_type": "SECURITY_ALERT", "source_agent": "sentinel", "rule_id": verdict.rule_id, "severity": verdict.severity, "preview": original_payload[:120], "ts": time.time()})})
...
def _consume_loop(stop_event: threading.Event) -> None:
    try:
        ...
        while not stop_event.is_set():
            try:
                messages = r.xreadgroup(_CONSUMER_GROUP, "sentinel-1", {SENTINEL_EVENTS_STREAM: ">"}, count=1, block=2000)
                if not messages:
                    continue
                for _stream, entries in messages:
                    for entry_id, data in entries:
                        ...
                        try:
                            ...
                            _logger.log("SENTINEL_SCAN_COMPLETE", {"blocked": verdict.blocked, "rule_id": verdict.rule_id, "source": verdict.source})
                        except Exception as exc:
...
def start() -> None:
    ...
    if _thread and _thread.is_alive():
        return
    _stop_event.clear()
    _thread = threading.Thread(target=_consume_loop, args=(_stop_event,), daemon=True, name="sentinel-agent")
    _thread.start()
