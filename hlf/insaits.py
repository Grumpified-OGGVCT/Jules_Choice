...
def fluent_decompile(bc_data: bytes, backend: str = "openrouter") -> str:
    """Decompile HLF bytecode into fluent English using an AI model.

    Args:
        bc_data: Raw bytes of an .hlb binary.
        backend: The provider to use via OllamaDispatcher.
    ...
    try:
        ...
        return "Error: Cannot load dispatcher for fluent_decompile."

    ops = decompile_bytecode(bc_data)
    prompt = f"Translate the following HLF bytecode operations into fluent, natural English steps:\n\n{ops}"

    req = InferenceRequest(
        prompt=prompt,
        model="qwen:7b",
        system="You are a senior AI translating HLF bytecode into clear, nuanced, fluent English steps. Provide only the translation.",
        metadata={"provider": backend}
    )

    dispatcher = get_dispatcher()
    try:
        # Attempt synchronous resolution safely
